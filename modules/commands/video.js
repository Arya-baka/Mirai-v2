module.exports.config = {
	...
	hasPermssion: 0,
	credits: "CatalizCS",
	description: "Phát video thông qua link YouTube ho?c t? khoá t?m ki?m",
...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage('Không th? g?i file v? dung lý?ng l?n hõn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400) return api.sendMessage('Không th? g?i file v? dung lý?ng l?n hõn 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
...
return api.sendMessage({attachment: imgthumnail, body: body}, event.threadID,(error, info) => global.client.handleReply.push({ 
  ...
  event.messageID);
      ...
      //api.sendMessage("Không th? x? l? request do d? phát sinh l?i: " + error.message, event.threadID, event.messageID);
